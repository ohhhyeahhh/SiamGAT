...
# from pysot.models.model_builder_single_gat_9_27 import ModelBuilder
from pysot.models.model_builder_single_gat import ModelBuilder
# from pysot.models.model_builder_single_gat_fixed_area import ModelBuilder
# from pysot.models.model_builder_single_car import ModelBuilder
...
# parser.add_argument('--cfg', type=str, default='../experiments/siamcar_alex/config.yaml',
#                     help='configuration of tracking')
parser.add_argument('--cfg', type=str, default='../experiments/siamgat_googlenet/config.yaml',
                    help='configuration of tracking')
...
def build_opt_lr(model, current_epoch=0):
    ...
    if current_epoch >= cfg.BACKBONE.TRAIN_EPOCH:
        ...
        # channel_reduce_layer's batchnorm set eval or train???
        # for m in getattr(model.backbone, layer).modules():
        #     if isinstance(m, nn.BatchNorm2d):
        #         m.train()

        # if cfg.TRAIN.CHANNEL:
            # # googlenet channel_reduce
            # if model.backbone.channel_reduce in model.backbone.modules():
            #     for param in model.backbone.channel_reduce.parameters():
            #         param.requires_grad = True
    ...
    if cfg.TRAIN.ATTENTION:
        if cfg.TRAIN.ATTENTION_NUM == 1:
            trainable_params += [{'params': model.attention.parameters(),
                                  'lr': cfg.TRAIN.BASE_LR}]
        else:
            for i in range(cfg.TRAIN.ATTENTION_NUM):
                trainable_params += [{'params': getattr(model, 'attention' + str(i)).parameters(),
                                      'lr': cfg.TRAIN.BASE_LR}]
        # trainable_params += [{'params': model.attention.parameters(),
        #                       'lr': cfg.TRAIN.BASE_LR}]

    if cfg.TRAIN.DOWN:
        # add down’ s parameters
