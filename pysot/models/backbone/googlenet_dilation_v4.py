...
class Inception3_dilation_v4(nn.Module):
    r"""
    GoogLeNet
    ...
    def __init__(self, transform_input=False):
        super(Inception3_dilation_v4, self).__init__()
        ...
        #
        self.channel_reduce_11 = nn.Sequential(
            nn.Conv2d(768, 256, kernel_size=1),
            nn.BatchNorm2d(256, eps=0.001),
        )

        self.channel_reduce_22 = nn.Sequential(
            nn.Conv2d(768, 256, kernel_size=3, dilation=(2, 2), padding=(2, 2)),
            nn.BatchNorm2d(256, eps=0.001),
        )
    ...
    def forward(self, x, bbox=torch.tensor([])):
        ...
        if len(bbox):
            x11 = self.channel_reduce_11(x)
            x22 = self.channel_reduce_22(x)

            assert bbox.shape[-1] == 4 and bbox.shape[0] == x.shape[0]
            stride = 8
            offset = 13
            mask = torch.zeros_like(x22)
