...
class Inception3(nn.Module):
    ...
    def forward(self, x, bbox=torch.tensor([])):
        ...
        if len(bbox):
            ...
            offset = 13
            # ########################ROI Align#############################
            # target_area = ((bbox - offset + stride / 2) / stride - 1).float()
            # ROI = RoIAlign((5, 5), spatial_scale=1, sampling_ratio=-1)
            # x = ROI(x.cuda(), [target_area.cuda()])
            # # ##############################################################

            mask = torch.zeros_like(x)

            roi = torch.round((bbox + 1 - offset + stride / 2) / stride - 1)
            # roi = ((bbox + 1 - offset + stride / 2) / stride - 1).float()
            # roi bbox start from:1
            # roi = ((bbox - offset + stride / 2) / stride - 1).float()
            # roi = torch.round((bbox - offset + stride / 2) / stride - 1)
            for i in range(x.shape[0]):
                ...
                mask[i, :, max(0, roi[i][1].int()): min(roi[i][3].int(), 12),
                        max(0, roi[i][0].int()): min(roi[i][2].int(), 12)] = 1
                #########################model_9_15############################
                # mask[i, :, max(0, torch.floor(roi[i][1]).int()): min(torch.ceil(roi[i][3]).int(), 12),
                #             max(0, torch.floor(roi[i][0]).int()): min(torch.ceil(roi[i][2]).int(), 12)] = 1
            x *= mask
        else:
            x = x[:, :, 4:-4, 4:-4]
        # x = x[:, :, self.crop_pad:-self.crop_pad, self.crop_pad:-self.crop_pad]
