...
from pysot.utils.xcorr import axcorr_depthwise
...
class ModelBuilder(nn.Module):
    ...
    def track(self, x):
        ...
        features = self.attention(xf[0], self.zf[0])
        for i in range(len(xf)-1):
            features_new = self.attention(xf[i+1], self.zf[i+1])
    ...
    def forward(self, data):
        ...
        features = self.attention(xf[0], zf[0])
        for i in range(len(xf)-1):
            features_new = self.attention(xf[i+1], zf[i+1])
            features = torch.cat([features, features_new], 1)
        # delete down
        features = self.down(features)
