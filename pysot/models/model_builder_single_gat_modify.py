...
from pysot.models.head.car_head_8_7 import CARHead
from pysot.models.neck import get_neck
from ..utils.location_grid import compute_locations
from pysot.utils.xcorr import xcorr_depthwise

"""
googlenet_modify: delete channel_reduce, so the last layer is relu
"""
...
class ModelBuilder(nn.Module):
    def __init__(self):
        ...
        # build car head
        # self.rpn_head = CARHead(cfg, 256)
        self.car_head = CARHead(cfg, 256)

        # build response map
        inchannel = int(192*4/cfg.TRAIN.ATTENTION_NUM)
        for i in range(cfg.TRAIN.ATTENTION_NUM):
            self.add_module('attention'+str(i), Graph_Attention_Union(inchannel, inchannel))
        ...
        #     nn.ConvTranspose2d(256 * 3, 256, 1, 1),
        #     nn.GroupNorm(32, 256),
        #     nn.ReLU()
        # )
    ...
    def track(self, x):
        ...
        channel = int(192*4 / cfg.TRAIN.ATTENTION_NUM)
        for i in range(cfg.TRAIN.ATTENTION_NUM):
            attention = getattr(self, 'attention' + str(i))
            if i == 0:
                features = attention(self.zf[:, i*channel: (i+1)*channel, :, :], xf[:, i*channel: (i+1)*channel, :, :])
            else:
                feature = attention(self.zf[:, i*channel: (i+1)*channel, :, :], xf[:, i*channel: (i+1)*channel, :, :])
    ...
    def forward(self, data):
        ...
        for i in range(cfg.TRAIN.ATTENTION_NUM):
            attention = getattr(self, 'attention' + str(i))
            if i == 0:
                features = attention(zf[:, i*channel: (i+1)*channel, :, :], xf[:, i*channel: (i+1)*channel, :, :])
            else:
                feature = attention(zf[:, i*channel: (i+1)*channel, :, :], xf[:, i*channel: (i+1)*channel, :, :])
                features = torch.cat([feature, features], 1)
