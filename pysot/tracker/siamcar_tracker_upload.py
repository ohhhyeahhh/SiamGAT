...
class SiamCARTracker(SiameseTracker):
    ...
    def track(self, img, hp):
        ...
        # self.show_cls_map(np.array([cls, cen]), x_crop)
        # get w h
    ...
    def show_cls_map(self, score_map, image):
        score = score_map.reshape(-1, 25, 25)
        x_image = image.permute(0, 2, 3, 1)[0].data.cpu().numpy()
        imgs = x_image
        for i in range(score.shape[0]):
            cls_show = cv2.cvtColor(cv2.resize(score[i].astype(np.float32), (255, 255), interpolation=cv2.INTER_CUBIC),
