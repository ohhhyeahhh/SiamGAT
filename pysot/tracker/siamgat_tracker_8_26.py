...
class SiamCARTracker(SiameseTracker):
    ...
    def _convert_cls(self, score):
        ...
        # score = (score - score.min())/score.ptp()
        return score
    ...
    def track(self, img, hp):
        ...
        cen_sig = torch.sigmoid(outputs['cen']).data.cpu().numpy().squeeze()
        ...
        hp_score_sig = hp_cls * cen_sig
        hp_score_sig_up = cv2.resize(hp_score_sig, (upsize, upsize), interpolation=cv2.INTER_CUBIC)
        ...
        p_cls_up = cv2.resize(p_cls, (upsize, upsize), interpolation=cv2.INTER_CUBIC)
        cen_sig_up = cv2.resize(cen_sig, (upsize, upsize), interpolation=cv2.INTER_CUBIC)
        p_score_up = p_cls_up * cen_sig_up

        # get center
        max_r_up, max_c_up, new_cx, new_cy, cen_crop_up = self.getCenter(hp_score_1_up, p_score_1_up, scale_score, lrtbs)
        # max_r_up, max_c_up, new_cx, new_cy, s = self.getCenter(hp_score_sig_up, hp_score_sig_up, scale_score, lrtbs)
        cen_crop = cv2.resize(cen_crop_up, (self.cfg.SCORE_SIZE, self.cfg.SCORE_SIZE), interpolation=cv2.INTER_CUBIC)
        # if args.vis:
        self.show_cls_map(np.array([cls, cen, cen_crop, hp_score_1]), x_crop)
        # get w h
